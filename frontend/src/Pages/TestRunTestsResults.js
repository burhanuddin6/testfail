...
import { useLocation, Link, useNavigate } from "react-router-dom";
import "../styles/TestRunTestsResults.css";
import { fetchSectionsAndCases } from "../api/TestCase";
...
const TestRunTestsResults = () => {
  ...
  useEffect(() => {
    const fetchTestCaseData = async () => {
      try {
        console.log("suite id in testRuntesResults page for sections and cases "+suiteId)
        const data = await fetchSectionsAndCases(suiteId);
        console.log("data fetched in testRuntesResults page for sections and cases "+ data);

        
        // Set sections and cases from the fetched data
        setSections(data.sections || []);
        setCases(data.cases || []);

        // Calculate test case statistics
        const updatedTestCaseData = data.cases.reduce(
          (acc, testCase) => {
            acc[testCase.status.toLowerCase()] += 1;
            return acc;
          },
          { passed: 0, blocked: 5, retest: 0, failed: 0, untested: 0 }
        );
