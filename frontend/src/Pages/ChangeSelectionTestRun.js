...
import { fetchSectionsAndCases } from '../api/TestCase.js';

const ChangeSelection = ({ suiteId, onCancel, setSelectionsInChild, setApiRes}) => {
    ...
    const [sections, setSections] = useState([{ title: "lolol" }]);

    useEffect(() => {
        const tbr = async () => {
            const sec = await fetchSectionsAndCases(suiteId);
            console.log(`Suite ID:${suiteId} \n sec: ${JSON.stringify(sec, null, 2)}`);
            setSections(sec);
            setApiRes(sec);
        };
        tbr();
    ...
    const onSend = () => {
        setSelectionsInChild(selectedItems2);
        onCancel();
    }
        ...
        <div className="ChangeSelection-overlay">
            <div className="ChangeSelection-content">
                ...
                <div className="ChangeSelection-body">
                    ...
                    <div className="ChangeSelection-main">
                        ...
                        <div className="case-list">
                            ...
                            {sections.map((item) => {
                                return selectedItems.includes(item.id) ? item.cases.map((subItem) => {
                                    return (
                                        <label>
                                            <input
                                                type="checkbox"
                                                // checked={selectedItems2.includes(item.title)}
                                                onChange={() => handleCheckboxChange2(subItem.id)}
                                            />
                                            {subItem.title}
                                        </label>);
                                }) : <></>
                ...
                <div className="ChangeSelection-footer">
                    <button className="ok-button" onClick={onSend}>OK</button>
